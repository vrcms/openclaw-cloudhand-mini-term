...
## 会话韧性与故障恢复 (Resilience)

为系统提供鲁棒性，应对网络抖动或临时离线：

### 1. 5 分钟会话保留
中继服务器在检测到本地端失联后，会**保留当前 Agent 会话 5 分钟**。
- **重连即恢复**：如果本地客户端在 5 分钟内重新连回并保持相同 Token，中继会自动清除清理计时器。
- **无感续接**：此时无需重新调用 `POST /agent/start`，直接继续 `GET /agent/status` 或发送指令即可。

### 2. PTY 热接管 (Hot Resuming)
如果必须重新调用 `/agent/start`（例如主 Agent 逻辑需要重置），本地驱动也会检查现有的 PTY 进程。
- **智能衔接**：只要本地托盘程序没关，新的 `start` 请求会直接“热接管”旧的 PTY 进程，对话历史得以保留。

### 3. 本地消息缓冲 (Message Backlog)
本地客户端具备消息缓冲区：
- **数据不丢失**：即使在 AI 执行期间网络断开，本地端也会将所有的终端日志、任务进度和最终结果存回队列。
- **上线补齐**：网络重连并认证成功后，系统会自动“补发”积压的所有消息，确保主 Agent 能看到连贯的执行过程。

### 4. 异常恢复 SOP (Error Recovery)
若主 Agent 发起 `/agent/send` 后网络中断或收到 5xx 错误，**严禁重试相同任务**（以免导致本地重复执行）。
- **标准动作**：应首先调用 `GET /agent/history`。
- **判断依据**：如果 history 中最新的消息是本地 Claude 的回复，说明任务已自动在后台完成并落库。
- **继续执行**：直接基于 history 中的结果进行后续验收或汇报。

### 🚨 绝对警示：严禁中间态清理 (Forbidden Auto-Stop)
- **对话即资产**：本地 PTY 进程是保存变量、缓存和上下文的**唯一容器**。
- **自杀行为**：严禁在每轮任务结束后调用 `/agent/stop`。一旦调用，PTY 进程会被物理销毁，所有的对话历史和临时变量将**彻底无法找回**。
- **状态切换而非停止**：完成任务后应停留在 `idle` 状态（由程序自动切换），主 Agent 只需要结束当前的 HTTP 对话槽位即可，**绝对禁止**清理后台 Worker。
...
- **归还裁决权**：验证达标后，应向人类用户（主人）详细汇报成果（包括已执行的验证操作），并询问“是否结束会话/清理资源”。
- **静默待命**：在得到人类明确授权前，必须保持在 `idle` 状态，以便人类进行人工二次核验。

### 4. 回复质量与排版美化 (Output Formatting)
由于本地端 Claude 在 PTY 这种有限的终端环境下输出，可能会导致 Markdown 语法错位、代码块泄露或表格排版异常。
- **强制排版审计**：主 Agent 收到本地回复后，**必须**先行审计其可阅读性。
- **二次美化重构**：严禁直接“刷屏”式透传原始终端输出。主 Agent 应将子智能体的干货提取出来，重新组织成结构清晰、排版专业（使用 Markdown 标题、列表、高亮快）的报告，再正式发送给主人。
- **专业汇报感**：给主人的内容应当像一份精美的文档，而不是一串凌乱的命令行输出。
...
6. **故障兜底**：若遇到非预期的 503 或 504 错误，建议主 Agent 首先尝试 `GET /agent/status` 查看是否由于网络瞬断引起，并利用上述“韧性机制”进行静默恢复。
